import React from 'react';
import { FaLightbulb } from 'react-icons/fa';
import { Bell } from 'lucide-react';
import { OceanButton } from '@/components/OceanButton';
import UpdateCard from './UpdateCard';
import { Suggestion } from '@/types/dashboard';

interface SuggestionsSectionProps {
  filteredSuggestions: Suggestion[];
  clearSearch: () => void;
  openSuggestionModal: () => void;
}

const SuggestionsSection: React.FC<SuggestionsSectionProps> = ({ 
  filteredSuggestions, 
  clearSearch,
  openSuggestionModal 
}) => {
  return (
    <div className="transform -translate-x-full opacity-0 animate-slideInFromLeft border border-[#0077b6]/30 bg-[#0c2e44]/80 backdrop-blur-sm rounded-xl overflow-hidden shadow-lg shadow-[#0077b6]/10">
      <div className="flex items-center border-b border-[#0077b6]/30 bg-gradient-to-r from-[#0c4c74] to-[#051e2f] p-4">
        <FaLightbulb className="text-[#00b4d8] h-6 w-6 mr-3" />
        <h2 className="text-xl font-bold text-white">User Suggestions</h2>
      </div>
      
      <div className="p-4">
        {filteredSuggestions.length > 0 ? (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3">
            {filteredSuggestions.map(suggestion => (
              <UpdateCard key={suggestion.id} suggestion={suggestion} />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center rounded-xl border border-[#0077b6]/30 bg-[#0c2e44]/50 py-12 text-center">
            <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[#051e2f]">
              <Bell className="h-8 w-8 text-[#90e0ef]" />
            </div>
            <h3 className="mb-2 text-xl font-semibold text-white">No suggestions found</h3>
            <p className="mb-6 text-[#ade8f4]">There are no suggestions matching your search criteria.</p>
            <OceanButton onClick={clearSearch} variant="outline">
              Clear search
            </OceanButton>
          </div>
        )}
      </div>
      
      {/* Add suggestion button for all users */}
      <div className="mt-6 flex justify-center sm:justify-end p-4">
        <button 
          onClick={openSuggestionModal}
          className="inline-flex items-center rounded-full bg-gradient-to-r from-[#0077b6] to-[#00b4d8] px-4 py-2 text-sm font-medium text-white shadow-md transition-all hover:from-[#00b4d8] hover:to-[#0077b6] hover:shadow-lg"
        >
          <FaLightbulb className="mr-2 h-4 w-4" />
          Share your idea
        </button>
      </div>
    </div>
  );
};

export default SuggestionsSection;
